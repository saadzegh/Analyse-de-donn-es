...
```{r, ehco=F, eval=T, fig.cap="\\label{fig:kmeans1}A completer"}
...
```{r ,fig.cap="\\label{fig:CAH1}A compléter v"}
# A COMPLETER
CH<-NULL
Kmax<-20
for (k in 2:Kmax){
...
On observe les resultats en Figure \ref{fig:nb_classes2}
...
```
En observant le graphique de l'inertie intra-classe, le "coude" apparaît autour de quatre classes. On décide donc d'en créer quatre, ce qui devrait permettre de capturer une quantité significative de variance.

Le coefficient de silhouette moyen pour quatre classes est légèrement plus élevé que pour d'autres choix de nombre de clusters, ce qui indique une meilleure séparation et cohésion des classes. 
...
```{r, echo=F, eval=T, fig.cap="\\label{fig:Kmeans2}à compléter"}
...
Expliquer qu'on fait ward !!

```{r, echo=FALSE, eval=TRUE, fig.cap="\\label{fig:dendrogramme}à compléter"}
# dendrogramme
...
```{r, echo=FALSE, eval=TRUE, fig.cap="\\label{fig:à completer}à completer"}
CH<-NULL
Kmax<-20
for (k in 2:Kmax){
...
```{r, echo=FALSE, eval=TRUE, fig.cap="\\label{fig:a completer}à completer", results='hide'}
...
```{r, echo=FALSE, eval=TRUE, fig.cap="\\label{fig: completer}àcompleter"}
...
```{r, echo=FALSE, eval=TRUE, fig.cap="\\label{fig:completer}à completer", results='hide'}
clust<-paste("Cl-K",reskmeans$cluster,sep="")
