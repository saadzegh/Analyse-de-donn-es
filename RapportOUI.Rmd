...
```
D'après la Figure \ref{fig:matricecorrelations},
...
La majorité des corrélations sont positives

on observe des corrélations fortes entre les variables des traitement T2 et T3 tandis que les variables du traitement T1 sont globalement peu corrélées aux autres variables.

Les variables mesurant le même traitement (T1, T2, ou T3) à des heures différentes ou pour des réplicats différents devraient présenter des corrélations positives, surtout si les mesures sont cohérentes.


Si certaines variables montrent des corrélations faibles ou négatives inattendues, cela peut indiquer une variabilité importante ou des effets expérimentaux particuliers.


Puisque T3 est une combinaison des traitements T1 et T2, 

on peut s'attendre à des corrélations significatives entre T3 et les variables de T1 et T2.
...
montrent la répartition des modalités des variables qualitatives (ExpT1, ExpT2, ExpT3)
...
Nous menons ici une analyse en composantes principales où les variables Tt_sH_Rr sont les individus décrits par les
gènes.

A enlever : Pour savoir si utilise l'ACP centrée ou l'ACP centrée reduite, on compare les pourcentages des variances cumulées sur les deux ACP. Car on ne peut pas utiliser des boxplots pour le déterminer (trop de variables)
