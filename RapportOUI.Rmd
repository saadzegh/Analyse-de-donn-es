...
Pour évaluer l'impact des traitements, on habille le graphe des variables par les résultats de décision (variables ExpT1, ExpT2 et ExpT3) en Figure \ref{fig:graph_var_hab}

```{r, echo=FALSE, eval=TRUE, fig.cap="\\label{fig:contributions}Contribution relative des individus à l'inertie sur les axes factoriels"}
...
```
ExpT1:
Les variables "Sous" se situent principalement dans la partie supérieure gauche du graphe, indiquant une contribution positive et significative à Dim2.
...
```{r, echo=FALSE, eval=TRUE, fig.cap="\\label{fig:indiv_hab}Habillage sur le graphe des individus en fonction du réplicat (gauche), du traitement (centre) et de l'heure (droite)"}
Aux <- rownames(respca$ind$coord)
#On choisit de ne pas afficher les points de moyenne, malgré leurs apports d'information, pour garder le même graph des individus que l'ACP
#Habillage sur le graphe des individus - Réplicats
replicats <- str_sub(Aux, 7, 8)
g3 <- fviz_pca_ind(respca, habillage = as.factor(replicats), label = "none", mean.point=FALSE)

#Habillage sur le graphe des individus - Traitements
traitements <- str_sub(Aux, 1, 2)
g4 <- fviz_pca_ind(respca, col.ind = as.factor(traitements), label = "none", mean.point=FALSE)

#Habillage sur le graphe des individus - Heures
heures <- str_sub(Aux, 4, 5)
g5 <- fviz_pca_ind(respca, col.ind = as.factor(heures), label = "none", mean.point=FALSE) 
...
Dès la deuxième heure, une séparation claire apparaît : la contribution par rapport à Dim1 devient opposée entre le premier traitement et les deux autres traitements.
...
Nous nous intéressons enfin à l'analyse des gènes du jeu de données.

## Analyse en composantes principales

Pour commencer, nous menons une analyse en composantes principales du jeu de données "DataExpMoy" contenant la moyenne des expressions sur les réplicats de chaque gène, pour chaque traitement et chaque heure. {\textbf{\textsf{R}}}
...
Voir boxplots figure \ref{fig:boxplotsquantitatives}.
...
La première composante principale (métavariable Dim1) est portée par T2 et T3 qui sont fortement colérées.
La seconde composante principale (métavriable dim2) est davantage portée par T1. On remarque que plus les heures avance, plus le traitement 1 se rapproche de l'axe de dim2.
