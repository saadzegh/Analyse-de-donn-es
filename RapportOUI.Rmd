...
Dans ce projet, on s'intéresse à la population des gènes d'une plante modèle. On souhaite analyser les données représentant les différences d'expression d'un certain nombre de gènes pour différents traitements appliqués.
...
```
Le jeu de données fourni est composé d'un échantillon de n=542 individus (gènes) et de 39 variables statistiques qui caractérisent les individus. On observe que les 36 premières variables sont des variables quantitatives continues (variables Tt_sh_Rr). Elles permettent d'évaluer les différences d'expression des gènes pour trois traitements appliqués (T1, T2 et T3) durant 6 heures et ceci pour deux réplicats biologiques (R1 et R2).

Les trois dernières variables (ExpT1, ExpT2 et ExpT3) sont des variables qualitatives nominales dont les trois modalités sont : "Non", "Sous" et "Sur". Ces dernières représentent pour chaque traitement la décision si le gène est sur-exprimé, sous-exprimé ou non-exprimé à 6 heures.
Les premières lignes du jeu de données (pour les variables T1_1h_R1 à T1_6h_R1 et la variable ExpT1) sont affichées dans la Table 1.
...
#commandes pour l'analyse du jeu de données
head(data)
str(data)
names(data)
colnames(data)
...
knitr::kable(t(df),caption="\\label{tab:tabfreq}Fréquences en pourcentages des variables qualitatives")
