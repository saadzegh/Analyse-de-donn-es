...
g1 <- fviz_pca_ind(respcav, habillage = as.factor(replicats), label = "none", mean.point=FALSE)

#Habillage sur le graphe des individus - Traitements
traitements <- str_sub(Aux, 1, 2)
g2 <- fviz_pca_ind(respcav, col.ind = as.factor(traitements), label = "none", mean.point=FALSE)

#Habillage sur le graphe des individus - Heures
heures <- str_sub(Aux, 4, 5)
g3 <- fviz_pca_ind(respcav, col.ind = as.factor(heures), label = "none", mean.point=FALSE) 
...
#corrplot(respcag$var$cor,method="ellipse")  #les corrélations des variables initiales avec toutes les méta-variables => Plus lisible (mais pas top)
