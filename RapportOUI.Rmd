...
Le cluster 1 est caractérisé par une réponse homogène aux traitements. Il regroupe principalement les observations des traitements T2 et T3, en particulier pour les heures avancées de ces derniers. On peut en déduire que les premiers effets de ces présentent une faible variabilité.
...
Sur le dendrogramme (Figure 13), l’axe vertical représente la distance/différences entre les gènes et leurs d'expressions sous différents traitements.
...
Puisqu'on analyse un même nombre de gènes entre chaque experience, on  utilise Ward comme mesure d'agrégation. Au vu de l'analyse des composantes principales, on cherche à former des classes sphériques avec des effectifs équivalents. 
...
Le grape des individus ne permet pas de distingier des groupe distincts sans les clusters. 
Les gènes du cluster 1 sont faiblement influencés par les traitements, présentant des variations minimes. Ils semblent principalement associés au traitement T1.
En ce qui concerne le cluster 2, il regroupe des gènes dont les réponses aux traitements sont modérées. 
Le cluster 3, quant à lui, contient des gènes fortement influencés par les traitements appliqués à des heures avancées. On note que sur les traitements T2 et T3, des variations significatives sont observées après plusieurs heures de traitement. 
Enfin, le cluster 4 se distingue par des gènes qui affichent des variations marquées dès le début du traitement, particulièrement en réponse à T2 et T3, traduisant une réaction rapide à ces traitements.
On a des résultats similaires aux K-Means, ce qui permet de valider l'interprétation.
...
Sur la Figure \ref{fig:alluvions1}, on observe que le quatrième clusters des Kmeans présente des gènes qui sont plus éparpillés dans la classification hiérarchique. On en conclut que la classification hiérarchique donne un résultat légèrement plus précis sur la classification des gènes? Excepté cela, on remarque que les clusters sont presque identiques, quelque soit la méthode utilisée.

### Comparaison des différentes méthodes d'ACP et de clustering des différentes experiences

```{r, echo=FALSE, eval=TRUE, fig.cap="\\label{fig: graph indiv sur_sous_non}Habillage du graphe des individus par expression des gènes sur chaque experience"}
...
```{r, echo=FALSE, eval=TRUE, fig.cap="\\label{fig:alluvions2}Diagramme d'alluvions des expressions génétiques dans les clusters du K-means", results='hide', fig.height=2.5}
clust<-paste("Cl-K",reskmeans$cluster,sep="")
...
```{r, echo=FALSE, eval=TRUE, fig.cap="\\label{fig:alluvions3}Diagramme d'alluvions des expressions génétiques dans les clusters de la classification hiérarchique", results='hide', fig.height=2.5}
clust<-paste("Cl-hiérarchique",ClustCH,sep="")
Tab<-melt(table(clust,dataACPg[,21]))
g3<-ggplot(Tab,aes(y=value,axis1=clust,axis2=Var2))+
  geom_alluvium(aes(fill=clust))+
...
On observe sur la Figure \ref{fig:alluvions2} les résultats des clustering suite aux Kmeans et ceux de la classification hiérarchique sur la Figure \ref{fig:alluvions3}. Ils permettent d'en arriver à la même conclusion: Les gènes répondent de manière distincte aux traitements. EXPT1 génère une faible réponse, avec des gènes sans réaction notables dans tous les clusters et en grand nombre. EXPT2 et EXPT3 induisent des réponses plus variées, avec une minorité quasimet négligeable de gène "Non".
