...
g1pie <- ggplot(df1, aes(x = "", y = value, fill = group)) + 
  ...
  geom_text(aes(label = scales::percent(value, accuracy = 0.1)), 
            ...
            size = 3) +
  theme_grey(base_size = 15) +  # Conserve le fond gris
  theme(
    panel.grid = element_blank(),      # Supprime les lignes de grille
    axis.title = element_blank(),      # Supprime les titres d'axes
...
# Fonction pour créer un dataframe avec les pourcentages formatés
create_df_with_percentage <- function(data_col) {
  counts <- table(data_col)
  total <- sum(counts)
  df <- data.frame(
    group = names(counts),
    value = as.vector(counts) / total,
    label = paste0(names(counts), " (", scales::percent(as.vector(counts) / total, accuracy = 0.1), ")")
  )
  return(df)
}

# Pour ExpT1
df1 <- create_df_with_percentage(data$ExpT1)
g1pie <- ggplot(df1, aes(x = "", y = value, fill = group)) + 
  geom_bar(width = 1, stat = "identity") +
  coord_polar("y", start = 0) +
  scale_fill_manual(values = scales::hue_pal()(length(df1$group)), labels = df1$label) +  # Ajout des pourcentages dans la légende
  theme_grey(base_size = 15) + 
  theme(
    panel.grid = element_blank(),
    axis.title = element_blank(),
...
# Pour ExpT2
df2 <- create_df_with_percentage(data$ExpT2)
g2pie <- ggplot(df2, aes(x = "", y = value, fill = group)) + 
  geom_bar(width = 1, stat = "identity") +
  coord_polar("y", start = 0) +
  scale_fill_manual(values = scales::hue_pal()(length(df2$group)), labels = df2$label) + 
  theme_grey(base_size = 15) + 
  theme(
    panel.grid = element_blank(),
    axis.title = element_blank(),
...
# Pour ExpT3
df3 <- create_df_with_percentage(data$ExpT3)
g3pie <- ggplot(df3, aes(x = "", y = value, fill = group)) + 
  geom_bar(width = 1, stat = "identity") +
  coord_polar("y", start = 0) +
  scale_fill_manual(values = scales::hue_pal()(length(df3$group)), labels = df3$label) + 
  theme_grey(base_size = 15) + 
  theme(
    panel.grid = element_blank(),
    axis.title = element_blank(),
...
g1pie <- ggplot(df1, aes(x = "", y = value, fill = group)) + 
  ...
  coord_polar("y", start = 0) +
  geom_text(aes(label = scales::percent(value, accuracy = 0.1)), 
            position = position_stack(vjust = 0.7),  # Décalage pour rendre les étiquettes plus visibles
            size = 4) +  # Taille du texte ajustée pour plus de lisibilité
  theme_grey(base_size = 15) + 
  theme(
    panel.grid = element_blank(),
    axis.title = element_blank(),
