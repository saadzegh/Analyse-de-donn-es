...
Pour expliquer ces changements, on effectue une analyse qui compare chacune des caractéristiques (replicat, traitement, heures) des 3 experiences, retranscrite dans la Figure \ref{fig:graph_ind_hab}.
...
### Détermination du nombre de classes nécéssaires

Tout d'abord, nous déterminons le nombre de classes nécéssaires au clustering à l'aide de deux critères distincts : l'étude de l'évolution de l'inertie intra-classe et l'étude de silhouette, pour lesquels nous affichons les résultats en Figure \ref{fig:nb_classes}.
```{r}
#Inertie intraclasse
set.seed(2)
Kmax<-15
...
```{r, echo=FALSE, eval=TRUE, fig.cap="\\label{fig:nb_classes}Evolution de l'inertie intra-classe et silhouette"}
#silhouette
...
On applique dans un premier temps la méthode des Kmeans qui permet d'obtenir les 3 clusters: {\textbf{\textsf{R}}} 
...
grid.arrange(g1,g2,g3,ncol=3)
```
Le choix du nombre de cluster n'est pas évident en voyant le pic du critère de Calinski-Harabasz. L'hyopthèse de 3 clusters faite lors des Kmeans n'est donc pas incohérente et peut être gardée pour la classification hierarchique.
...
On décide de faire un clustering sur les données brutes car, quelques soit le gènes, on étudie les même composantes. Le fait de centrer réduire ferait perdre de l'information.
...
On applique la méthode des Kmeans qui permet d'obtenir les 4 clusters: {\textbf{\textsf{R}}} 
...
Au vu du nombre important de gènes, on ne peut pas faire une conjecture du nombre de cluster à vu d'œil grâce au dendrogramme.
...
Sur le  graphe du critère de Calinski-Harabasz, on observe une forte diminution du critère CH au début (entre 1 et 5 clusters), suivie d'une diminution plus progressive pour un nombre de clusters supérieur.
Le point de "coude" semble être autour de 4 clusters, où la pente change notablement avant de devenir plus linéaire.
Sur la base de ce graphique et des Kmeans fait précédemment, 4 clusters ont choisis pour la classification hiérarchique. Ils maximisent le critère CH tout en évitant une complexité inutile liée à un trop grand nombre de clusters.

La hauteur où le dendrogramme présente 4 grandes branches, les gènes sont donc classés en 4 clusters.
...
Les deux méthodes (Kmeans et classification hiérarchique) ont identifié 4 clusters optimaux.
On observe que le 4eme clusters des Kmeans présente des gènes qui sont plus éparpillés dans la classification hiérarchique. On en conclut que la classification hiérarchique donne un résultat légèrement plus précis sur la classification des gènes? Excepté cela, on remarque que les clusters sont presque identiques, quelque soit la méthode utilisée.
...
g1<-ggplot(Tab,aes(y=value,axis1=clust,axis2=Var2))+
  ...
  theme(legend.position = "none")
```
Les graphiques d'individus obtenus suite à l'ACP permettent de montrer:

Figure 15.1 (ExpT1) :

La majorité des gènes sous T1 sont sans évolution “Non”, ce qui reflète une faible réponse des gènes à ce traitement.
Les groupes "Sous" et "Sur" sont très peu représentés, ce qui prouve l'idée que le traitement T1 a un effet limité sur l'expression génétique.

Figure 15.2 (ExpT2) :

Les gènes “Sous” et “Sur” sont mieux représentés et se distribuent de manière plus large, marquant une plus forte réponse au traitement T2 par rapport à T1.
Les gènes "Non" exprimés sont significativement moins nombreux qu’avec T1.

Figure 15.3 (ExpT3) :

La distribution est similaire à T2 mais avec une augmentation du groupe "Sur" exprimé.
Cela confirme que T3, étant une combinaison de T1 et T2, amplifie les effets de T2 tout en masquant en grande partie l’effet de T1.
